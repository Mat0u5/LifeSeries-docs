...
Soulmates in Double Life no longer get damage ticked when you take damage while blocking with a shield.
Improved the `/boogeyman list` command.

----------

Fixed a bug where the client would crash when the hearts in the config entry were negative.
Made some Secret Life configs heart config entries.

Added config entries for seconds and minutes, which show the readable time in the popup above the textbox.
Lots of configs from Limited Life, Secret Life, Wild Life are now these types.

----------

Added a Click to expand/collapse text to text config entries which are group headers.
...
The timers and cooldowns in the mod no longer show up when you press F1.
...
The creative mode ignores blacklist config now works even when players in creative open chests or try to pick up items from the ground.
...
Added the 'announce_soulmates' config to Double Life, which shows the soulmate name when they are rolled. Default value is false.
Added the 'boogeyman_announce_outcome' global config, which broadcasts a message in chat when the boogeyman fails or is cured. Default value is true.
When the session ends and the boogeyman is offline, it now actually removes their lives instead of telling the adming to do it when the player logs on again.
...
Added the 'broadcast_secret_keeper' Secret Life Config, which shows a message in chat when anyone succeeds, fails or rerolls a task. Default value is false.
...
In Double Life, all soulmates are now given a tag, `soulmate_<index>` where the index is determined by sorting all players by UUID and assigning consecutive numbers to each pair (starting from 1)
Wild Life zombies are periodically checked to be alive, and if so, they are internally removed from the zombies list and given back the default number of hearts.
...
Swapped Real Life and Simple Life order in the choose april fools season menu.
...
Added the 'show_health_below_name' global config, which allows you to show the health below the player usename. Default value is false.
...
Session timer length now persists even if you change the series.
Shortened the unjustified kill message, because it was too long.
...
The blacklists are now compatible with modded items.
...
Using `/lives reset <player>` or `/lives resetAll` now sets the players' lives to the default, instead of letting them be unassigned.
You can now be dark green in Limited Life when you are above the default number of seconds.

----------

Added the 'watchers_in_tab' global config, which controls whether the Watchers are shown in the tab list. Default value is true.
...
Some season specific commands no longer even get suggested when you are not playing that season.
You now cannot give a life to your soulmate in Double Life
...
All minutes config entries can now be properly saved.
...
Merged the '/limitedlife time' command into the '/lives' command. Finally. (Needs the time argument in Limited Life, otherwise accepts normal numbers.)
Changed the '/lastlife rollLives' command into the '/lives rollLives' command.

----------

You can now use multiple targets for '/task set/fail/succeed/reroll'.
You can now use '/task fail/succeed/reroll' when the session is not on.
...
Snails no longer chase players in creative mode.
Snails now properly despawn when you're dead.

----------

You can no longer evade snails by using the astral projection superpower.
Mimicry now works with astral projection. clones, same with player disguise.
You can no longer get bigger when the size shifting wildcard is active while you're in astral projection.
You no longer get put back into survival when the session ends, and you had the astral projection superpower.

----------

When a player dies or logs off, their superpower now gets deactivated.
Reworked the backend logic for if a player is allowed to attack another player.
In seasons where you gain a reward for killing, you no longer do when it's an unjustified kill.
...
Added the 'soulbound_effects' Double Life config, which makes your effects be shared with your soulmate. Default value is false.
...
The inventory no longer gets updated when you swing your hand.
When you join a session late and the boogeyman has been chosen, but the session has already ended, you can no longer become the boogeyman.
Using '/lives reset' now syncs lives in Double Life properly.
When the final feath message config is empty, the message now gets fully skipped.

----------

Watchers are no longer affected by the Hunger wildcard nor the Size Shifting wildcard.
Players now get notified when they become / are no longer a Watcher.
